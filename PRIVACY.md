...
If you inspect the APK yourself you will also see a fifth line:
`com.rober.photoframe.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`. That one is generated by the
AndroidX libraries, not written by us. It is a *signature-level permission the app defines for
itself*, used so that broadcast receivers registered at runtime cannot be reached by other
apps. It grants RetroFrame no access to anything, and only an app signed with the same key
could hold it. It is mentioned here because the verification steps below will show it to you,
and an unexplained extra permission is exactly the kind of thing that should make you
suspicious of a document like this one.
